...
* After drawing a line, if the end point is not a pre-existing point, then it is automatically choosen as the start point of the next line. If you do not want to draw more lines, use *cancel* as described above.
...
* Debugging: auto-reconnect to Unity server - With this active, if a bug occurres the program will automatically try reconnect the link between SketchUp and VR.
